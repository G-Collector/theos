...
{
	static LLCachedControl<std::string> settingName(gSavedSettings, "OSBeamColorFile");
...
void lggBeamMaps::fireCurrentBeams(LLPointer<LLHUDEffectSpiral> mBeam, LLColor4U rgb)
{
	...
	static LLCachedControl<std::string> colorf(gSavedSettings, "OSBeamColorFile");
...
{
	static LLCachedControl<std::string> settingNameCached(gSavedSettings, "OSBeamShape");
	...
	if(settingName != lastFileName)
	{
		...
		if(settingName != "===OFF===" && settingName != "")
		{
			...
			for(int i = 0; i < myPicture.size(); i++)
			{
				...
				static LLCachedControl<F32> FSBeamShapeScale(gSavedSettings, "OSBeamShapeScale");
			...
			static LLCachedControl<F32> FSMaxBeamsPerSecond(gSavedSettings, "OSMaxBeamsPerSecond");
...
{
	static LLCachedControl<bool> FSParticleChat(gSavedSettings, "OSParticleChat");
	if(FSParticleChat)
	{
		if(sPartsNow != FALSE)
		{
