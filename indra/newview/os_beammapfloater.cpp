...
public:
	...
	static void onClickSaveContinued(AIFilePicker* filepicker,void* data);
	static void onClickClear(void* data);
	static void onClickLoad(void* data);
	static void onClickLoadContinued(AIFilePicker* filepicker,void* data);

private:

	void onBackgroundChange(LLUICtrl* ctrl);
...
{
	LLUICtrlFactory::getInstance()->buildFloater(this, "os_floater_beamshape.xml");
...
BOOL lggBeamMapFloater::postBuild(void)
{
	...
	//getChild<LLColorSwatchCtrl>("back_color_swatch")->setCommitCallback(onBackgroundChange,this);

	getChild<LLColorSwatchCtrl>("back_color_swatch")->setCommitCallback(boost::bind(&lggBeamMapFloater::onBackgroundChange, this, _1));
...
{
	empanel = (OSPanelPreferences*)data;
	if(empanel)
	{
...
void lggBeamMapFloater::onBackgroundChange(LLUICtrl* ctrl)
{	
	LLColorSwatchCtrl* cctrl = (LLColorSwatchCtrl*)ctrl;
	
	if(cctrl)
	{
		getChild<LLPanel>("beamshape_draw")->setBackgroundColor(cctrl->get());
	}
...
void lggBeamMapFloater::onClickSave(void* data)
{
	...
	filepicker->open("myNewBeam.xml", FFSAVE_XML, gDirUtilp->getExpandedFilename(LL_PATH_APP_SETTINGS, "beams"), "");
	filepicker->run(boost::bind(&onClickSaveContinued, filepicker, data));
}

void lggBeamMapFloater::onClickSaveContinued(AIFilePicker* filepicker, void* data)
{	
	...
	filename = path_name2 +gDirUtilp->getBaseFileName(filepicker->getFilename());
	...
	export_file.close();
	gSavedSettings.setString("OSBeamShape",gDirUtilp->getBaseFileName(filename,true));
...
{
	AIFilePicker* filepicker = AIFilePicker::create();
	filepicker->open(FFLOAD_XML, gDirUtilp->getExpandedFilename(LL_PATH_APP_SETTINGS, "beams"), "", false);
	filepicker->run(boost::bind(&onClickLoadContinued, filepicker, data));
}

void lggBeamMapFloater::onClickLoadContinued(AIFilePicker* filepicker, void* data)
{
	if(!filepicker->hasFilename())
	{
		return;
