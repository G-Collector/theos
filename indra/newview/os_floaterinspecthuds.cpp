	...
	mIDsConnected(false)
{
	...
	LLUICtrlFactory::getInstance()->buildFloater(this, "os_floater_inspect_huds.xml");
	...
	if(scroll_itemp)
	{
		...
		while(it_textures != it_textures_end)
		{
			if(seen_textures.count((*it_textures)) == 0)
			{
    			...
    			OSInvTools::addItem((*it_textures).asString(), (int)LLAssetType::AT_TEXTURE, *it_textures, true);
			}
			it_textures++;
		}
	}
...
void LLFloaterAttachments::dumpTaskInvFile(const std::string& title, const std::string &filename)
{
	...
	if(ifs.good())
	{
		...
		LLUUID parent_id = gInventory.findCategoryUUIDForType(LLFolderType::FT_NOTECARD);
