...
:	LLFloater(),
	mEditID(LLUUID::null)
{
	LLUICtrlFactory::getInstance()->buildFloater(this, "os_floater_vfs.xml");
}
LLFloaterVFS::~LLFloaterVFS()
{
	sInstance = NULL;
...
void LLFloaterVFS::onClickCopyUUID(void* user_data)
{
	...
	gViewerWindow->getWindow()->copyTextToClipboard(utf8str_to_wstring(file.mID.asString()));
}
// static
void LLFloaterVFS::onClickItem(void* user_data)
{
	LLFloaterVFS* floaterp = (LLFloaterVFS*)user_data;
	entry file = floaterp->getEditEntry();
	OSInvTools::addItem(file.mName, (int)file.mType, file.mID, true);
}
// static
