...
void FollowPrimList::setObjectID(LLUUID id)
{
	if (id.isNull())
	{
		llwarns << "Trying to set null id1" << llendl;
	}
	mObjectsID = id;
}
	...
	if (regionp)
	{
		...
		for (i = 0; i < count; ++i)
		{
			...
			if (avatarp)
			{
				...
				S32 agent_x = llround((F32)fmod(position.mdV[VX], (F64)REGION_WIDTH_METERS));
				S32 agent_y = llround((F32)fmod(position.mdV[VY], (F64)REGION_WIDTH_METERS));
				S32 agent_z = llround((F32)position.mdV[VZ]);
				childSetValue("X", LLSD(agent_x));
				childSetValue("Y", LLSD(agent_y));
				childSetValue("Z", LLSD(agent_z));
