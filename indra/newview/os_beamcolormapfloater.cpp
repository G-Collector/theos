...
{
	LLUICtrlFactory::getInstance()->buildFloater(this, "os_floater_beamcolor.xml");
...
void lggBeamColorMapFloater::onClickSave(void* data)
{
	...
	filepicker->open("myNewBeamColor.xml", FFSAVE_XML, gDirUtilp->getExpandedFilename(LL_PATH_APP_SETTINGS, "beamsColors"), "");
	filepicker->run(boost::bind(&onClickSaveContinued, filepicker, data));
}

void lggBeamColorMapFloater::onClickSaveContinued(AIFilePicker* filepicker, void* data)
{	
	...
	gSavedSettings.setString("OSBeamColorFile",gDirUtilp->getBaseFileName(filename,true));
...
{
	AIFilePicker* filepicker = AIFilePicker::create();
	filepicker->open(FFLOAD_XML, gDirUtilp->getExpandedFilename(LL_PATH_APP_SETTINGS, "beamsColors"), "", false);
	filepicker->run(boost::bind(&onClickLoadContinued, filepicker, data));
}

void lggBeamColorMapFloater::onClickLoadContinued(AIFilePicker* filepicker, void* data)
{
	if(!filepicker->hasFilename())
	{
		return;
